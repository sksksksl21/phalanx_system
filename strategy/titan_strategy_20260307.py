...
class TitanStrategy:
    def __init__(self):
        ...
        # =====================================================================
        # [Params]15,3.25,18,15,3,150,0.5,3,True,True,True,True,True,True,67,30,0.8,200,50,1.05,
        # =====================================================================
        self.params = {
            # --- 15분봉(Intraday) 설정 ---
            "atr_period": 15,
            "atr_multiplier": 3.25,
            "adx_threshold": 18,  # (옵션) 필터로만 사용
            "rsi_upper": 67,      # (옵션) 필터로만 사용
            "rsi_lower": 30,      # (옵션) 필터로만 사용
            "vol_factor": 0.8,    # 볼륨 신뢰도 점수 기준(게이트 아님)
            "ema_intraday": 200,
            ...
            "context_lookback": 150,        # sweep/trigger 유효기간
            "retest_tolerance_atr": 0.5,    # retest 레벨 허용오차(ATR 비율)
            "use_daily_filter": True,       # 일봉 EMA 필터
            "use_vol_filter": True,         # ✅ Gate 제거. 신뢰도 점수 계산만.

            # --- Safety / Direction ---
            "use_st_dir_filter": True,      # ST 방향/가격 위치 불일치 시 차단

            # --- NEW: Structure Confirmation (CHOCH/BOS) ---
            "use_structure_confirm": True,  # ✅ CHOCH + BOS/TrendState 결합
            "structure_min_pivots": 3,      # 피봇 2개 이상 있을 때만 구조판정

            # --- NEW: Volatility Regime Gate (no volume gate) ---
            "use_vol_regime_gate": True,
            "atr_regime_len": 50,           # ATR 평균(레짐)
            "atr_regime_factor": 1.05,      # ATR / ATR_MA > factor 면 "움직일 장"
            "atr_slope_gate": True,         # ATR 상승(모멘텀)도 같이 요구
        }
    ...
    def analyze(self, symbol, df):
        ...
        # ---------------------------------------------------------
        # ✅ NOTE: gate 계산은 유지하지만, "신호 차단"에는 사용하지 않는다.
        vol_gate_ok = True
