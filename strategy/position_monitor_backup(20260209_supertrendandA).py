...
# =========================
    ...
    def check_conditions(
        ...
        if mode == "next" and bool(position.get("sl_breached", False)):
            ...
            if (candle_time_s is None) or (breached_at_s is None) or (breached_at_s != candle_time_s):
                ...
                if exec_sl is not None:
                    ...
                    return "EXIT", float(exec_sl), "STOP_LOSS", _safe_float(position.get("sl", None))
        # -----------------------------
        ...
        if strat == "supertrend":
            ...
            # (profit_trigger 제거 → P1 통합을 "전략 전체"에 적용하려면 PATCH 2를 적용)
            # 여기선 기존 호환 유지
            cand = _tighten_sl(_candidate_profit_lock(), current_sl)  # noqa: F821 (기존 함수가 있을 때)
            cand = _maybe_apply_constraints(cand, current_sl)
            if cand is not None:
                new_sl = float(cand)
                action = "UPDATE_SL"
