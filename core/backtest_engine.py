...
class BacktestEngine:
    ...
    def _reset_pre_analyze_log(self):
        path = os.path.join(self.root_dir, "backtest_pre_analyze.log")
        with open(path, "w", encoding="utf-8") as f:
            f.write("")
        logger.info(f"📝 PRE_ANALYZE_LOG_RESET | {path}")


    def _log_pre_analyze_backtest(self, sym: str, current_time, past_data: pd.DataFrame,
                                signal, sl_price, tp_price):
        if past_data is None or (not isinstance(past_data, pd.DataFrame)) or past_data.empty:
            return
        ...
        snap_cols = [
            ...
            "atr", "adx",
            "ema_intra", "ema_daily", "ema_daily_ok",
            "st_dir", "st_val",
            "trend_up", "trend_down",
            "pullback_long_recent", "pullback_short_recent",
        ...
        for c in snap_cols:
            if c in last.index:
                snap[c] = _sv(last.get(c))

        line1 = (
            f"{current_time} 🧪 PRE_ANALYZE_BT | {sym} "
            f"t={current_time} rows={len(past_data)} "
            f"snap={json.dumps(snap, ensure_ascii=False)}"
        )
        line2 = (
            f"{current_time} 🧠 ANALYZE_RESULT_BT | {sym} "
            f"signal={str(signal).upper() if signal else None} "
            f"sl={_sv(sl_price)} tp={_sv(tp_price)} "
    ...
    def _prepare_daily_context(self, symbols):
        ...
        need_days = int(max(100, daily_len + 75))
        loaded = 0
    ...
    def run(self, show_report=False):
        ...
        # ✅ PRE_ANALYZE 비교용 별도 로그 초기화
        self._reset_pre_analyze_log()
        ...
        for current_time in sim_times:
            ...
            for sym in fixed_symbols:
                ...
                # ✅ LIVE의 PRE_ANALYZE / ANALYZE_RESULT와 대조할 백테 전용 로그
                self._log_pre_analyze_backtest(
                    sym=sym,
