...
class LiveEngine:
    ...
    def _get_manage_row(self, sym, authority_time):
        ...
        ✅ 반환은 항상 pd.Series로 통일하고,
        ✅ .name 에 authority_time을 부여해 downstream가 안전하게 동작하도록 한다.
        """
        df = self.data_map.get(sym)
        ...
        if isinstance(df, pd.DataFrame) and not df.empty:
            if authority_time in df.index:
                ...
                # row가 Series가 아닐 수도 있으니 방어
                if isinstance(row, pd.Series):
                    return row
                try:
                    return pd.Series(dict(row), name=authority_time)
                except Exception:
                    pass
            ...
            try:
                ...
                if isinstance(last, pd.Series):
                    # last.name은 df index로 이미 timestamp임
                    return last
                return pd.Series(dict(last), name=df.index[-1])
            except Exception:
                pass

        # -----------------------------
        # ticker fallback (Series로 반환)
        # -----------------------------
        try:
            t = self.executor.fetch_ticker(sym)
            px = float(t.get("last") or t.get("close") or 0)
            if px > 0:
                # authority_time을 name으로 강제
                ...
                return pd.Series(
                    {
                        ...
                        "high": px,
                        "low": px,
                        # atr/st_val/adx는 최소 안전값(ARMOR entry_atr 고정/stop hit 방어용)
                        "atr": px * 0.01,
                        "st_val": None,
                        "adx": 0.0,
                    },
    ...
    def _process_existing_position_live(self, sym, curr_row):
        ...
        # -------------------------------------------------
        # ✅ curr_row normalize: Series/dict 모두 허용
        # -------------------------------------------------
        row_series = None
        if isinstance(curr_row, pd.Series):
            ...
            # authority_time을 알 수 없으니 now로 대체 (manage row가 Series로 오면 이 분기 거의 안 탐)
        ...
        # -------------------------------------------------
        # ✅ UPDATE_SL / EXIT 이하 로직은 기존 코드 그대로 유지
        # -------------------------------------------------
        if action == "UPDATE_SL":
            applied_kind = "NONE"
    ...
    # ---- EXIT 로직 이하: 기존 코드 그대로 (여기선 생략) ----
    # ⚠️ 너의 원본에서 EXIT 처리 블록을 그대로 붙여넣으면 됨.
    # (이 함수는 def 단위 교체이므로, EXIT 블록 전체를 기존 그대로 유지)
    ...
    def run_once_live(self):
        ...
        # ✅ 매 루프(15m) 1회 reconcile (경량/안전 가드 포함)
        try:
            self.reconcile_positions()
        except Exception as e:
