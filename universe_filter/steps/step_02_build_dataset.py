...
def main() -> None:
    ...
    # apply blacklist from strategy_settings.blacklist (root config)
    bl = set()
    try:
        bl_cfg = (cfg or {}).get("strategy_settings", {}).get("blacklist", []) or []
        bl = {str(x).strip().upper() for x in bl_cfg}
    except Exception:
        bl = set()
    ...
    _log(f"🚫 blacklist applied | before={before_n} after={after_n} removed={before_n-after_n}")
